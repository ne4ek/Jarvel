...
class UserChatComposeMailService:
    ...
    async def change_mail_sending_delay(self, message: types.Message, state: FSMContext):
        ...
        new_mail_send_delay = await self.mailing_assistant.change_mail_send_delay(messages,
                                                                                 data.get("company_code"))
    ...
    async def get_telegram_message(self, state: FSMContext):
        ...
        telegram_message = self.mailing_assistant.compose_telegram_filling_message(mail, go_to_main_menu_button=False)
        telegram_message["keyboard"].inline_keyboard.append([InlineKeyboardButton(text="В главное меню", callback_data="user_go_to main_menu"),
    ...
    async def get_initial_compose_menu(self, state: FSMContext, user_id: int, bot_message: types.Message):
        data = await state.get_data()
        ...
        telegram_message = self.mailing_assistant.compose_telegram_filling_message(mail, go_to_main_menu_button=False)
        telegram_message["keyboard"].inline_keyboard.append([InlineKeyboardButton(text="В главное меню", callback_data="user_go_to main_menu"),
                                                             InlineKeyboardButton(text="В меню почты", callback_data=f"user_go_to mail_menu_company_code:{company_code}")])
        await state.update_data({"mail": {"entity": mail, "message": bot_message}})
        return telegram_message
